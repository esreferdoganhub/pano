...
if (!firebaseConfig.apiKey) {
    ...
    configRef.child('viewPassword').on('value', (snapshot) => {
        viewPassword = snapshot.val() || "1300"; // Fallback to 1300 if not set

        // Check Session
        if (sessionStorage.getItem('visitorAuth') === viewPassword) {
            showBoard();
        }
    });
    ...
    loginForm.addEventListener('submit', (e) => {
        e.preventDefault();
        const input = document.getElementById('visitorPass').value;
        if (input === viewPassword) {
            sessionStorage.setItem('visitorAuth', viewPassword);
            showBoard();
        } else {
    ...
    function startPostListener() {
        ...
        marked.setOptions({
            ...
            sanitize: false,    // Allow HTML tags (IMPORTANT for your use case)
            smartLists: true,
            smartypants: false
        });
        ...
        postsRef.limitToLast(20).on('value', (snapshot) => {
            ...
            renderPosts('ALL', feedEl, allPosts);
        });
        ...
        document.querySelectorAll('.filter-btn').forEach(btn => {
            btn.addEventListener('click', () => {
                ...
                // Render
                renderPosts(btn.dataset.category, feedEl, allPosts);
            });
        });
    }

    function renderPosts(category, feedEl, allPosts) {
        ...
        filteredPosts.forEach(post => {
            ...
            // Render content (supports both HTML and Markdown)
            const contentDiv = card.querySelector('.post-content');

            // Check if content looks like HTML (contains < and > tags)
